            ...
            if (!file.exists()) {
                if (path.contains(".")) {
                    ...
                    try {
                        ...
                        if (showOutput) out("Created a file: " + path);
                    } catch (FileNotFoundException fnfe) {
                        if (showOutput) err("Couldn't create a file: " + fnfe.getMessage());
                    ...
                    if (file.mkdir()) {
                        if (showOutput) out("Created directory: " + path);
                    } else {
                        if (showOutput) err("Couldn't create directory: " + path);
                    }
                }
            } else {
                if (showOutput) out("There is already: " + path);
            }
        } else {
            if (showOutput) err("Given path has no content.");
        }
    }
